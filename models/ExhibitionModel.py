...
class ExhibitionModel:
    ...
    def getFilterExs(self,xfilter):
        ...
        if xfilter == 'ongoing':
            ...
            qr1 = self.cur.execute('select * from reshma.pvfexhibition left join mansi.cat on ex_cat_id = cat_id where ex_end_date < current_timestamp order by ex_end_date')
        entries = qr1.fetchall()
        exs = list()
        for entry in entries:
            ex = list(entry)
